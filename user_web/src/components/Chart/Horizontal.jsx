...
export default function ChartHorizontal({
  ...
  // These calculations ensure all regions fit within the measured container.
  ...
  useEffect(() => {
    ...
    for (
      ...
      timeBaseContext.fillText(label, Math.floor(labelPositionX), 5);
  ...
  useEffect(() => {
    ...
    const padding = 6;
    timeOverlayContext.font = "13px 'JetBrains Mono', 'Fira Code', monospace";
    const labelWidth = Math.max(
      80, // Minimum chip width for consistent UI
    ...
    timeOverlayContext.lineWidth = 1; // Border width
    roundRectPath(
      timeOverlayContext,
      labelRectX,
      labelRectY,
  ...
  useEffect(() => {
    ...
    const hoverChipWidth = Math.max(
      60, // Minimum width for visual consistency
    ...
    priceOverlayContext.lineWidth = 1;
    roundRectPath(
      priceOverlayContext,
      hoverChipX,
      hoverChipY,
  ...
  useEffect(() => {
     ...
     */
    const calculateHoverIndex = (pointerX) => {
      const barCount = mappedBars.length;
      ...
      if (pointerX < chartStart || pointerX >= chartEnd) {
      ...
      const relativeX = pointerX - chartStart - xOffRef.current;
    ...
    const chartPriceFromPointer = (pointerY) => {
      const innerPointerY = pointerY - TIME_SCALE_H;
      const clampedY = clamp(innerPointerY, 0, glHeight);
      const effectiveHeight = glHeight - TOP - BOTTOM;
    ...
    const handlePointerMove = (event) => {
      ...
      if (selectorDragRef.current) {
        ...
        if (innerPointerY >= 0 && innerPointerY <= glHeight) {
          setMouseYInner(innerPointerY);
        }
        return;
      }
      ...
      if (
        innerPointerY >= 0 &&
        innerPointerY <= glHeight &&
        pointerX >= 0 &&
        pointerX <= chartAreaW + PRICE_SCALE_W
      ) {
        setMouseYInner(innerPointerY);
      } else setMouseYInner(null);

      // Map X pixel position to the specific bar index
      setHoverIndex(calculateHoverIndex(pointerX));
    };
    ...
    const handlePointerDown = (event) => {
      ...
      if (selectorActive) {
        ...
        if (innerPointerY < 0 || innerPointerY > glHeight) return;
        const rawPrice = chartPriceFromPointer(pointerY);
    ...
    const handlePointerDrag = (event) => {
      ...
      lastPointerX = event.clientX;
      velRef.current = deltaX; // Record velocity for inertia release
      ...
      setHoverIndex(
        latestMouse.inside ? calculateHoverIndex(latestMouse.x) : null
      );
    };
    ...
    const handleWheel = (event) => {
      ...
      if (Math.abs(deltaYRaw) > Math.abs(deltaXRaw) || event.ctrlKey) {
        ...
        setBarWidth((previousWidth) => {
          ...
          setOffsetX((currentOffset) => {
            ...
            const barAtMouseRaw =
              (pointerX - (LEFT + currentOffset)) / oldStride;
            ...
            const newOffset = pointerX - LEFT - barAtMouse * newStride;
      ...
      // PAN LOGIC: Horizontal scroll or touchpad swipe
        ...
        setOffsetX((currentOffset) => {
          const next = currentOffset - deltaXRaw * 0.7; // 0.7 dampening factor
          return clamp(next, minOffsetLimit - colStride * 2, colStride * 2);
        });
        velRef.current = -deltaXRaw * 0.7; // Update inertia
      }
